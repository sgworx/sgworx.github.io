...
class CareerGraph3D {
    ...
    setupCamera() {
        ...
        this.camera.position.set(0.4, 1.0, -1.0); // Zoomed in a bit more, more to the right, lower angle
    ...
    createAxes() {
        const axisLength = 0.6; // Reduced axis length to create distance from text
        ...
        // Reposition labels: Design at top - all rotated 
        this.createLabel('Design', new THREE.Vector3(0, 0, labelDistance), '180');      // Top (positive Z)
        this.createLabel('Fabrication', new THREE.Vector3(-labelDistance, 0, 0), '90'); // Left (negative X)
        this.createLabel('Tech/Product', new THREE.Vector3(labelDistance, 0, 0), '-180'); // Right (positive X)
        this.createLabel('AI', new THREE.Vector3(0, 0, -labelDistance), '360');         // Bottom (negative Z)
    }
    ...
    movePersonToYear(year) {
        ...
        const yearPositions = {
            ...
            '2023': { x: -0.15, z: -0.15 },     // Slight exploration of AI (-Z) while still very Fabrication heavy
            '2024': { x: 0.1, z: -0.25 },      // Between AI (-Z) and Fabrication (+X)
